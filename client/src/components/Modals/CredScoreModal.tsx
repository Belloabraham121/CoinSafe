        ...
        {!isSuccess ? (
          <div className="flex flex-col items-center space-y-6 text-center">
            ...
            <div className="my-8">
              ...
              <p className="text-center text-sm text-gray-400">Out of 100</p>
            </div>

            <p className="">Claim your NFT as a proof of your cred score</p>
            <Button
              className="text-black bg-white hover:bg-white/90  px-8 rounded-[2rem]"
              disabled={isMinting}
              onClick={handleMint}
            >
              Claim NFT{" "}
              {isMinting && <Loader2 className="ml-1 w-4 h-4 animate-spin" />}
            </Button>
          </div>
        ) : (
          <div className="">
            ...
            {tnxhash && (
              <Link
                to={`https://sepolia.basescan.org/tx/${tnxhash}`}
                className="text-[#79E7BA] underline"
              >
