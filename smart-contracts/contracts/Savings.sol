...
contract Savings is ReentrancyGuard {
     ...
     * @param _sawiTokenAddress The address of the LISK token
     * @param _safuTokenAddress The address of the SAFU token
    */
    constructor(address _erc20TokenAddress, address _sawiTokenAddress, address _safuTokenAddress) {
        owner = msg.sender;
        
        if (_erc20TokenAddress == address(0) || _sawiTokenAddress == address(0) || _safuTokenAddress == address(0)) 
            revert AddressZeroDetected();

        acceptedTokensAddresses[TokenType.USDC] = _erc20TokenAddress;
        acceptedTokensAddresses[TokenType.SAWI] = _sawiTokenAddress;
        acceptedTokensAddresses[TokenType.SAFU] = _safuTokenAddress;

        acceptedTokens[_erc20TokenAddress] = true;
        acceptedTokens[_sawiTokenAddress] = true;
...
// ======================================= BALANCES ==========================================
    ...
    */
    function getUserBalances(address _user) external view returns (address[] memory, uint256[] memory) {
        ...
        tokens[0] = acceptedTokensAddresses[TokenType.USDC];
        tokens[1] = acceptedTokensAddresses[TokenType.SAWI];
...
// =================================== ADD TRANSACTION =====================================
    ...
    */
    function addTransaction(string memory _type, address _token, uint256 _amount) internal {
        ...
        emit TransactionHistoryUpdated(msg.sender, txCount, newTransaction.id, _token, _type, _amount, block.timestamp);
    }
